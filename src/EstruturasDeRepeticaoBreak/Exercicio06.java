package EstruturasDeRepeticaoBreak;

public class Exercicio06 {
    public static void main(String[] args) {
        // Dado o valor de um curso de programação de R$ 12.000, descubra todas as formas de parcelamento.
        // A condição é que cada parcela seja de no mínimo R$ 600.
        double valorCurso = 12000;
        for (int parcela = 1; parcela <= valorCurso; parcela++) {
            double valorParcela = valorCurso / parcela;
            if (valorParcela < 600) {
                break;
            }
            System.out.println("Total de parcelas: " + parcela + " R$ " + valorParcela);
        }
    }
}
